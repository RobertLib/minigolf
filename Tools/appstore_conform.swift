...
//
//      resolution   iPhone 6.9" 886 × 1920, iPad 13" 1200 × 1600 (portrait)
...
//  find out. The native device resolution is *not* accepted — 6.9" previews are
//  886 × 1920, nothing else — and Level 4.0 is a real ceiling, so a preview at
//  the device's own size comes out as Level 5.0 and is refused. Silence does not
//  satisfy the audio requirement either: AAC compresses digital silence to about
//  2 kbps, two orders of magnitude under the 256 kbps asked for, and a preview
//  with no usable audio is treated as an unsupported audio configuration. Hence
//  the music bed — pass a file and it is looped under the whole cut.
//
...
let fps: Int32 = 30
// Apple's band is 10–12 Mbps and the ask has to sit above it to land inside it:
// the encoder treats this as a ceiling to stay under, and gameplay at these
// sizes does not need all of it. 14 measures 10.2–10.9 across the four
// previews; 13 left the quietest of them at 9.8, just under the floor.
let videoBitRate = 14_000_000
...
func transfer(from output: AVAssetReaderOutput, to input: AVAssetWriterInput,
    ...
    let queue = DispatchQueue(label: "cz.rob.minigolf.appstore-conform")
...
func encodeVideo(asset: AVAsset, track: AVAssetTrack, to outURL: URL) async throws {
    ...
    let input = AVAssetWriterInput(mediaType: .video, outputSettings: [
        ...
        AVVideoCompressionPropertiesKey: [
            ...
            // A keyframe a second. Connect seeks the file to build the poster
            // frame, and a long GOP has bitten this pipeline before.
